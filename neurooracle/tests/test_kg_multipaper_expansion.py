...
import pytest
from neurooracle.src import kg_multipaper_expansion as repair
...
def fixture(monkeypatch):
    ...
    kg.serialization_metadata['multipaper_claim_expansion']=dict(version=repair.VERSION,rules=rules)
...
def test_invalid_policy_or_target_fails_before_mutation(monkeypatch,damage):
    ...
    nid=after[0]['metadata']['subject_id'];rules=kg.serialization_metadata['multipaper_claim_expansion']['rules']
...
def test_legacy_and_current_source_forms_reimport_to_same_shared_target(monkeypatch):
    kg,before,after,_=fixture(monkeypatch);rules=kg.serialization_metadata['multipaper_claim_expansion']['rules']
...
def test_prior_policy_remains_available_for_unrelated_owning_source(monkeypatch):
    from neurooracle.tests.test_kg_multipaper_claim_repair import fixture as previous_fixture
    from neurooracle.src import kg_multipaper_claim_repair as previous
    kg,_,_,_=fixture(monkeypatch)
    prior_kg,before,after,_=previous_fixture(monkeypatch)
    policy=deepcopy(prior_kg.serialization_metadata['multipaper_claim_repair'])
    for rule in policy['rules']:rule['pmid']+='9'
    monkeypatch.setattr(previous,'APPROVED_RULESET_DIGEST',digest(policy['rules']))
    kg.serialization_metadata['multipaper_claim_repair']=policy
