...
import pytest
from neurooracle.src import kg_multipaper_broadening as repair
...
def fixture(monkeypatch):
    ...
    kg.serialization_metadata['multipaper_claim_broadening']=dict(version=repair.VERSION,rules=rules)
...
def test_invalid_policy_or_target_fails_before_mutation(monkeypatch,damage):
    ...
    nid=after[0]['metadata']['subject_id'];rules=kg.serialization_metadata['multipaper_claim_broadening']['rules']
...
def test_legacy_and_current_source_forms_reimport_to_same_shared_target(monkeypatch):
    kg,before,after,_=fixture(monkeypatch);rules=kg.serialization_metadata['multipaper_claim_broadening']['rules']
...
def test_latest_source_rule_takes_priority_over_matching_r78_policy(monkeypatch):
    from neurooracle.tests.test_kg_multipaper_semantics import fixture as prior_fixture
    kg,before,after,_=fixture(monkeypatch)
    prior_kg,_,prior_after,_=prior_fixture(monkeypatch)
    kg.serialization_metadata['multipaper_claim_semantics']=deepcopy(prior_kg.serialization_metadata['multipaper_claim_semantics'])
