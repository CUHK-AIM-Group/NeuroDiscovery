...
from apply_kg_observational_semantics import LiteralTransform
...
from neurooracle.tests.test_kg_observational_semantics import fixture
from neurooracle.tests.test_kg_literal_endpoint_repair import edges


def stream_fixture(cid):
    row,reviews,proof=fixture(cid);event,current=repair.reviewed_claim(row,[],proof,reviews)
    owned=edges(row);owned[-1][1]['metadata'].update(negated=False,original_predicate=repair.SPECS[cid]['original_predicate'])
    ...
    plan=dict(events=[event],edge_events=repair.reviewed_edges(cid,row,current,owned),public_source_proof=proof,claim_reviews=reviews,
        new_literals=[],existing_targets={nid:digest(n) for nid,n in nodes.items()})
    source=[('metadata',None,{}),*[('node',nid,n) for nid,n in nodes.items()],('node',cid,row),*[('edge',o,e) for o,e in owned]]
    return source,plan


def inverse(records,plan):
    event=plan['events'][0];ed={e['ordinal']:e for e in plan['edge_events']};hashes={k:hashlib.sha256() for k in ('nodes','edges')}
...
def test_full_source_inverse_and_catalog_without_new_nodes_or_edges(tmp_path,cid):
    source,plan=stream_fixture(cid);before=deepcopy(source);t=LiteralTransform(plan);out=BytesIO();catalog=tmp_path/'catalog.jsonl'
    ...
    assert result['counts']==dict(nodes=3,claims=1,edges=3)
    assert verify_catalog_and_graph(iter(current),result,catalog)['shared_relation_index_complete']


@pytest.mark.parametrize('kind',['extra_reference','target_changed','public_proof','claim_review','missing_reference'])
def test_forward_rejects_incomplete_or_stale_observation_scope(kind):
    source,plan=stream_fixture(repair.REBOX)
    ...
    elif kind=='missing_reference':source.pop()
    else:plan['claim_reviews']={}
    with pytest.raises(ValueError):list(LiteralTransform(plan).records(iter(source)))


@pytest.mark.parametrize('kind',['raw','audit','stats','edge_source','design','endpoint'])
def test_independent_inverse_rejects_unapproved_evidence_or_design(kind):
    source,plan=stream_fixture(repair.REBOX);current=list(LiteralTransform(plan).records(iter(source)))
    claim=next(r for k,c,r in current if c==repair.REBOX)
    if kind=='raw':claim['metadata']['raw_text']='changed original'
    elif kind=='audit':claim['metadata']['scope_reaudit']['decision']='revalidated'
    elif kind=='stats':claim['metadata']['evidence']['sample_size']=74
    elif kind=='edge_source':current[-1][2]['source']='another paper'
    elif kind=='design':claim['metadata']['evidence']['methodology']='between-arm superiority'
    else:claim['metadata']['subject_id']='new endpoint'
