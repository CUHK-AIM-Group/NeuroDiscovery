...
import pytest
from neurooracle.src import kg_multipaper_reconciliation as repair
...
def fixture(monkeypatch):
    ...
    kg.serialization_metadata['multipaper_claim_reconciliation']=dict(version=repair.VERSION,rules=rules)
...
def test_invalid_policy_or_target_fails_before_mutation(monkeypatch,damage):
    ...
    nid=after[0]['metadata']['subject_id'];rules=kg.serialization_metadata['multipaper_claim_reconciliation']['rules']
...
def test_legacy_and_current_source_forms_reimport_to_same_shared_target(monkeypatch):
    kg,before,after,_=fixture(monkeypatch);rules=kg.serialization_metadata['multipaper_claim_reconciliation']['rules']
...
def test_source_bound_association_calibration_preserves_observation_and_edges(monkeypatch,old_predicate):
    ...
    monkeypatch.setattr(repair,'APPROVED_RULESET_DIGEST',digest(rules))
    kg.serialization_metadata['multipaper_claim_reconciliation']['rules']=rules
