"""R40 isolated scoped-structure engine, with independent added-about proof.

Adapted from the accepted R37 engine without editing historical frozen code.
Only this current manual batch is written; the closed night window stays closed.
"""
import argparse
...
from apply_kg_clinical_literal_reuse import FILES as PREVIOUS_FILES
...
from neurooracle.src.kg_scoped_structure import (literal_node, reviewed_edges, reviewed_added_about,
    edge_owner, apply_edge, change_claim)
from neurooracle.src import kg_scoped_structure as repair
from plan_kg_scoped_structure import public_proof
...
OUTPUT=journal.OUTPUT/"round40_scoped_structure"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".scoped.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES, Path(__file__), *[journal.REPO/p for p in (
    "neurooracle/scripts/plan_kg_scoped_structure.py", "neurooracle/src/kg_scoped_structure.py",
    "neurooracle/scripts/inspect_kg_scoped_structure.py", "neurooracle/scripts/inspect_kg_explicit_mri_mentions.py",
    "neurooracle/tests/test_kg_scoped_structure.py", "neurooracle/tests/test_kg_scoped_structure_pipeline.py",
    "neurooracle/tests/test_kg_scoped_structure_ingestion.py")]]))

def reviewed_claim(row, changes):
    return repair.reviewed_claim(row, changes, PUBLIC_PROOF)

def reverse_claim(row, event):
    original=repair.reverse_claim(row,event)
    reproduced,_=reviewed_claim(original,event["changes"])
    require(all(event[k]==v for k,v in reproduced.items()),"independent scoped source review differs")
    return original
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=600 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
...
class LiteralTransform:
    ...
    def records(self,records):
        for kind,key,row in records:
            ...
            if kind=="node":
                ...
                if key in self.events:
                    ...
                    reproduced,_=reviewed_claim(row,event["changes"])
        ...
        for item in self.plan["added_about_edges"]:
            yield "edge",str(item["ordinal"]),deepcopy(item["row"])
        added=[]
        for cid,row in self.originals.items():
            added.extend(reviewed_added_about(cid,row,change_claim(row,self.events[cid]),self.refs[cid]))
        require(added==[a["proof"] for a in self.plan["added_about_edges"]],"new about independent source proof differs")
        require([a["row"] for a in self.plan["added_about_edges"]]==[a["row"] for a in added],"added about row differs")
        reproduced=[]
        for cid,row in self.originals.items():
...
def load_plan(baseline):
    ...
    require(plan["version"]=="kg.scoped_structure.v1" and plan["status"]=="REVIEWED_NOT_APPLIED","wrong R40 plan")
    require(plan["graph"]==baseline["current_graph"] and plan["source_acceptance"]==baseline["current_acceptance"] and plan["detail_store"]==baseline["current_detail_store"],"source boundary differs")
    require(plan["repaired_gene_endpoints"]+plan["held_endpoints"]==plan["gene_endpoint_source_count"]==456,"endpoint decision closure differs")
    for item in [*plan["code"],plan["remaining_queue"],plan["source_inspection"],plan["mri_source_inspection"]]:small_check(item)
    for path in (plan["source_inspection"]["path"],plan["mri_source_inspection"]["path"]):
        inspection=journal.read_json(path);require(inspection["graph"]==plan["graph"],"inspection not current")
        small_check(inspection["code"])
    require(public_proof()==plan["public_proof"]==PUBLIC_PROOF,"public scientific proof changed")
    require([a["ordinal"] for a in plan["added_about_edges"]]==list(range(baseline["counts"]["edges"]+1,baseline["counts"]["edges"]+1+len(plan["added_about_edges"]))),"only appended about edges permitted")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R40结构闭合、显式MRI测量和来源方向修复",execution_mode="manual_user_continued",updated_at=journal.utc_now(),
        active_process=dict(kind="literal_endpoint_repair",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
    ...
    try:
        ...
        expected_counts["edges"]+=len(plan["added_about_edges"])
...
def validate():
    ...
    added={r["ordinal"]:r for r in plan["added_about_edges"]};seen_added=set()
    ...
    def independent(records):
        ...
        for kind,key,row in records:
            ...
            if kind=="node":
                ...
                if int(key) in added:
                    require(row==added[int(key)]["row"] and digest(row)==added[int(key)]["proof"]["row_sha256"],"unapproved added about edge")
                    seen_added.add(int(key));original=None
                elif int(key) in edges:
                    original=apply_edge(row,edges[int(key)],reverse=True); seen_edges.add(int(key))
                if original is not None and edge_owner(original) in events: original_refs[edge_owner(original)].append((int(key),original))
            if kind!="metadata":
    ...
    require(seen_nodes==set(events) and seen_edges==set(edges) and seen_new==set(new) and seen_added==set(added),"repair closure differs")
    require({k:v.hexdigest() for k,v in inverse.items()}==state["source_record_digests"],"inverse original-record digests differ")
    reproduced=[]
    for cid,row in original_nodes.items(): reproduced.extend(reviewed_edges(cid,row,change_claim(row,events[cid]),original_refs[cid]))
    ...
    for cid,row in original_nodes.items():
        added_proofs.extend(reviewed_added_about(cid,row,change_claim(row,events[cid]),original_refs[cid]))
    require(added_proofs==[a["proof"] for a in plan["added_about_edges"]],"independent appended about proof differs")
    ...
    checks.update(exact_approved_literal_endpoint_transform=True,inverse_reproduces_all_source_node_and_edge_record_digests=True,
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,
        publication_status_witnesses_validated=True,only_source_proved_direction_repair=True,
        missing_about_reconstructed_from_same_claim_and_existing_neutral_about=True,
        no_new_scientific_assertion_edges=True,all_other_scientific_values_quotes_and_conditions_preserved=True)
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="literal_endpoint_repair" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    changed_counts=dict(claim_nodes=plan["changed_claims"],endpoint_ids=plan["changed_endpoints"],edge_records=plan["changed_edges"],
        ...
        added_about_edges=len(plan["added_about_edges"]),repaired_gene_endpoints=plan["repaired_gene_endpoints"],
        source_proven_direction_repairs=plan["direction_repaired_claims"])
    receipt=dict(status="CURRENT_SCOPED_STRUCTURE_REPAIR_APPLIED",at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R40结构闭合、显式MRI指标和方向字段已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        next_steps=["完成当前结构/影像/方向修复；继续阅读R40报告和当前端点分层。",
            "同名但类型/范围冲突的节点不强并，R39旧临床锚点仍需明细联合迁移。",
            "既有语义和结构队列、书目冲突继续保留；无模型、训练、自动化重建或正式full_v2同步。"])
    journal.atomic_json(journal.OUTPUT/"CAMPAIGN.json",campaign)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="literal_endpoint_repair","unexpected active batch")
