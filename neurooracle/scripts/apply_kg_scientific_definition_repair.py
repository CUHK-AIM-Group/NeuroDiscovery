"""R57 own-source scientific definition corrections with all-record inverse proof."""
import argparse
...
from apply_kg_existing_literal_reuse import FILES as PREVIOUS_FILES
from plan_kg_scientific_definition_repair import inspection_bridge, incidence_bridge, source_inputs
...
from neurooracle.src.kg_scientific_definition_repair import (literal_node, reviewed_claim,
...
OUTPUT=journal.OUTPUT/"round57_scientific_definition_repair"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".scientific-definition.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES,Path(__file__),*[journal.REPO/p for p in (
    "neurooracle/src/kg_scientific_definition_repair.py","neurooracle/scripts/plan_kg_scientific_definition_repair.py",
    "neurooracle/scripts/inspect_kg_scientific_definition_scope.py","neurooracle/scripts/fetch_kg_scientific_definition_sources.py",
    "neurooracle/tests/test_kg_scientific_definition_inspection.py","neurooracle/tests/test_kg_scientific_definition_repair.py",
    "neurooracle/tests/test_kg_scientific_definition_plan.py","neurooracle/tests/test_kg_scientific_definition_pipeline.py",
    "neurooracle/scripts/report_kg_existing_literal_reuse.py","neurooracle/tests/test_kg_existing_literal_reuse_report.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=1160 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
...
def load_plan(baseline):
    from neurooracle.src.kg_scientific_definition_repair import SPECS,ENIGMA,FES
    fp=journal.fingerprint(OUTPUT/"PLAN.json");plan=journal.read_json(fp["path"])
    require(plan["status"]=="REVIEWED_NOT_APPLIED" and plan["version"]=="kg.scientific_definition_repair.v1"
        and plan["relation_grouping"]==POLICY,"wrong scientific correction policy")
    ...
    for item in [*plan["code"],*[plan[k] for k in ("remaining_queue","source_review","prior_identity_plan","existing_incidence_evidence")],
        *plan["public_manifests"],plan["public_abstracts"],*plan["public_fulltexts"].values()]:small_check(item)
    review_fp,inspection,reviews,manifests,abstracts,fulltexts,proof=source_inputs()
    ...
    targets=set(plan['reused_target_ids'])
    require(incidence_bridge(rows(plan['existing_incidence_evidence']['path']),
        [r for r in rows(inspection['artifacts']['CURRENT_TARGET_INCIDENCES.jsonl']['path']) if r['node_id'] in targets],
        journal.read_json(plan['prior_identity_plan']['path']),targets)==plan['prior_target_incidence_additions'],
        'prior target incidence bridge differs')
    require(manifests==plan["public_manifests"] and abstracts==plan["public_abstracts"] and fulltexts==plan["public_fulltexts"]
        and proof==plan["public_source_proof"],"own source proof differs")
    require({cid:reviews[cid] for cid in SPECS}==plan["claim_reviews"]
        and all(reviews[cid]==r for cid,r in plan["unmodified_sample_size_reviews"].items()),"finite current reviews changed")
    require({e["claim_id"] for e in plan["events"]}==set(SPECS) and len(plan["events"])==plan["changed_claims"]==8,"finite claim scope differs")
    require(sum(len(e["changes"]) for e in plan["events"])==plan["changed_endpoints"]==9,"endpoint scope differs")
    require(plan["repaired_queued_endpoints"]+plan["held_endpoints"]==plan["expanded_review_endpoints"]==2206
        and plan["original_watchlist_449_repaired"]==2 and plan["original_watchlist_449_remaining_after"]==1,"queue scope differs")
    require(plan["added_literal_nodes"]==len(plan["new_literals"])==3 and plan["reused_existing_nodes"]==2,"concept scope differs")
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R57原文支持的科学定义纠正：8条claim及独立全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="scientific_definition_repair",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
...
def validate():
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,publication_status_witnesses_validated=True,
        finite_own_source_scientific_field_corrections_reproduced=True,complete_existing_target_incidence_scope_reproduced=True,
        no_existing_concept_metadata_change=True,no_inferred_literal_semantic_type=True,
        exact_approved_scientific_definition_transform=True,inverse_reproduces_all_source_node_and_edge_record_digests=True,
        ...
        old_scientific_audit_not_revalidated=True,no_metadata_fields_added=True,
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="scientific_definition_repair" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    from neurooracle.src.kg_scientific_definition_repair import BACE,ACC,ENIGMA
    require(set(ch["claim_id"] for ch in plan["events"]) & set(scope["current_claim_hashes"])=={BACE,ACC},"unapproved scientific scope overlap")
    from neurooracle.src.kg_scientific_definition_repair import update_scope_findings
    scope=update_scope_findings(scope,plan)
    ...
    scope["latest_scientific_definition_plan"]=state["plan"]
    ...
    receipt=dict(status="CURRENT_SCIENTIFIC_DEFINITION_REPAIR_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R57八条科学定义及关系表达已有限修正并验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R57_ACCEPTED_CONTINUE_EXISTING_IDENTITY_AND_SCIENTIFIC_SCOPE_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["8条claim按原文纠正定义；历史449修复2处、仍剩1处，ENIGMA复合客体另列科学待核。",
            "均值、总和和分别两侧测量不混并；DPD统一表达但保留两份旧审核，论文来源不重复计数。",
            "17旧语义不变；原始截断摘录、复合客体、聚合不明与样本分母另列待核，无模型训练/full_v2同步。"])
    summary=dict(graph=graph,original_watchlist=449,original_watchlist_remaining_before=3,
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R57",
        latest_completed_runtime_batch="R57",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="scientific_definition_repair","unexpected active batch")
