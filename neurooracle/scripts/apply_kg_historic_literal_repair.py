"""R48 finite complete-mention identities with full inverse and current-reference proof."""
import argparse
...
from apply_kg_research_statement_retirement import FILES as PREVIOUS_FILES
from plan_kg_historic_literal_repair import inspection_bridge
...
from neurooracle.src.kg_historic_literal_repair import (literal_node, reviewed_claim,
...
OUTPUT=journal.OUTPUT/"round48_historic_literal_repair"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".historic-literal.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES, Path(__file__), *[journal.REPO/p for p in (
    "neurooracle/scripts/plan_kg_historic_literal_repair.py", "neurooracle/src/kg_historic_literal_repair.py",
    "neurooracle/tests/test_kg_historic_literal_repair.py", "neurooracle/tests/test_kg_historic_literal_pipeline.py",
    "neurooracle/tests/test_kg_historic_literal_plan.py",
    "neurooracle/scripts/inspect_kg_historic_literal_candidates.py", "neurooracle/tests/test_kg_historic_literal_inspection.py",
    "neurooracle/scripts/report_kg_research_statement_retirement.py", "neurooracle/tests/test_kg_research_retirement_report.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=900 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
...
def load_plan(baseline):
    ...
    require(plan["status"]=="REVIEWED_NOT_APPLIED" and plan["graph"]==baseline["current_graph"],"plan/source advanced")
    require(plan["source_acceptance"]==baseline["current_acceptance"] and plan["detail_store"]==baseline["current_detail_store"],"plan baseline differs")
    for item in plan["code"]: small_check(item)
    small_check(plan["remaining_queue"])
    require(plan["version"]=="kg.historic_complete_literal.v1" and plan["relation_grouping"]==POLICY,"wrong repair policy")
    small_check(plan["source_review"])
    review=journal.read_json(plan["source_review"]["path"])
    inspection_bridge(review,baseline,journal.read_json(baseline["current_acceptance"]["path"]))
    require(plan["historical_projection_rebound_by_fresh_current_claim_gene_node_incidence_and_full_source_hashes"]
        and plan["source_full_sha_verified"] and plan["source_census_full_sha_verified"],"current rebound missing")
    for item in (plan["existing_name_collision_evidence"],plan["decline_evidence"]): small_check(item)
    from neurooracle.src.kg_historic_literal_repair import review_key
    actual={review_key(r["claim_id"],r["side"]):r for r in rows(review["artifacts"]["CURRENT_ENDPOINT_ADJUDICATION.jsonl"]["path"])}
    require(actual==plan["endpoint_reviews"],"finite historic allowlist changed")
    for item in [review["code"],*review["artifacts"].values()]: small_check(item)
    require(plan["changed_endpoints"]+plan["held_endpoints"]==plan["expanded_review_endpoints"] and plan["changed_claims"]>0,"gene endpoint scope differs")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R48历史完整描述误指向批量纠正：全引用、科学字段不变及全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="historic_literal_repair",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
...
def validate():
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,publication_status_witnesses_validated=True,
        finite_historic_allowlist_and_current_references_reproduced=True,no_inferred_literal_semantic_type=True,
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="historic_literal_repair" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    receipt=dict(status="CURRENT_HISTORIC_LITERAL_REPAIR_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R48历史完整描述的基因误指向及共享索引已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R48_ACCEPTED_CONTINUE_SCOPE_EXISTING_LITERAL_AND_OTHER_ENDPOINT_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["已批量恢复历史完整描述身份；旧449队列与扩大5934队列分别记账，不把待核当确认错误。",
            "保留类型/分子机制/别名身份/混合引用待核；不进行名称截短或模糊合并。",
            "17条旧语义及产前、BACE1、ACC、小鼠VCP范围继续保留；无模型训练或正式full_v2同步。"])
    summary=dict(graph=graph,original_watchlist=449,original_watchlist_remaining_before=447,
        original_watchlist_repaired_this_batch=plan["changed_endpoints"],
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R48",
        latest_completed_runtime_batch="R48",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="historic_literal_repair","unexpected active batch")
