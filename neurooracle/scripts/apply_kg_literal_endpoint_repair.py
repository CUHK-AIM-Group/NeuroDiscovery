"""R37 atomic literal endpoint repair with independent inverse/full-graph proof."""
import argparse
...
from apply_kg_explicit_pmid_provenance import authorities, publication_row, FILES as PREVIOUS_FILES
...
from neurooracle.src.kg_literal_endpoint_repair import (literal_node, reviewed_claim,
...
TEMP=SOURCE.with_name(SOURCE.name+".literal.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES, Path(__file__), *[journal.REPO/p for p in (
    "neurooracle/scripts/plan_kg_literal_endpoint_repair.py", "neurooracle/src/kg_literal_endpoint_repair.py",
    "neurooracle/tests/test_kg_literal_endpoint_repair.py", "neurooracle/tests/test_kg_literal_endpoint_pipeline.py",
    "neurooracle/scripts/reconcile_kg_current_control.py", "neurooracle/tests/test_kg_control_reconciliation.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=490 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
...
def load_plan(baseline):
    ...
    require(plan["changed_endpoints"]+plan["held_endpoints"]==1206 and plan["changed_claims"]>0,"gene endpoint scope differs")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R37完整影像指标脱离错误基因节点：全引用及全图验收",updated_at=journal.utc_now(),
...
def apply():
    ...
    receipt=dict(status="CURRENT_LITERAL_ENDPOINT_REPAIR_APPLIED",at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R37完整指标身份、错误基因引用、共享索引及当前普查已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        next_steps=["已修复有完整明确影像类型和全引用证据的错误基因指向；不新增metadata字段。",
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R37",
        latest_completed_runtime_batch="R37",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260909.json",night)
