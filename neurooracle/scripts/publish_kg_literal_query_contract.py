...
No graph writes or new scientific acceptance. The R37 validator executes the
...
def flags_from_completed_proof(receipt,validated,state):
    require(receipt["status"]=="CURRENT_LITERAL_ENDPOINT_REPAIR_APPLIED","wrong batch")
...
def main():
    ...
    require(journal.fingerprint(receipt_path)==c["current_acceptance"],"R37 not current")
    ...
    extra=[journal.fingerprint(Path(__file__)),journal.fingerprint(journal.REPO/"neurooracle/tests/test_kg_literal_query_contract.py")]
