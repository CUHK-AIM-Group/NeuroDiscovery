"""R55 finite existing-name reuse with independent all-source and incidence inversion."""
import argparse
...
from apply_kg_expanded_literal_repair import FILES as PREVIOUS_FILES
from plan_kg_existing_literal_reuse import inspection_bridge, source_inputs
...
from neurooracle.src.kg_existing_literal_reuse import (literal_node, reviewed_claim,
    reviewed_edges, edge_owner, apply_edge, reverse_claim)
...
OUTPUT=journal.OUTPUT/"round55_existing_literal_reuse"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".existing-literal-reuse.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES,Path(__file__),*[journal.REPO/p for p in (
    "neurooracle/src/kg_existing_literal_reuse.py","neurooracle/scripts/plan_kg_existing_literal_reuse.py",
    "neurooracle/scripts/inspect_kg_existing_literal_reuse.py","neurooracle/tests/test_kg_existing_literal_reuse_inspection.py",
    "neurooracle/tests/test_kg_existing_literal_reuse.py","neurooracle/tests/test_kg_existing_literal_reuse_plan.py",
    "neurooracle/tests/test_kg_existing_literal_reuse_pipeline.py",
    "neurooracle/scripts/report_kg_expanded_literal_repair.py","neurooracle/tests/test_kg_expanded_literal_report.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=1100 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
        ...
        for kind,key,row in records:
            ...
            if kind=="node":
                ...
                if key in self.events:
                    ...
                    reproduced,_=reviewed_claim(row,event["changes"],self.plan["gene_witnesses"],self.plan["endpoint_reviews"])
...
def load_plan(baseline):
    ...
    require(plan["status"]=="REVIEWED_NOT_APPLIED" and plan["version"]=="kg.existing_literal_reuse.v1"
        and plan["relation_grouping"]==POLICY,"wrong reuse policy")
    ...
    for item in [*plan["code"],*[plan[k] for k in ("remaining_queue","source_review","prior_identity_plan","existing_incidence_evidence")]]:small_check(item)
    review_fp,inspection,prior,reviews,targets,incidences=source_inputs()
    require(review_fp==plan["source_review"] and inspection["pending_identity_plan"]==plan["prior_identity_plan"],"review binding differs")
    inspection_bridge(inspection,baseline,journal.read_json(baseline["current_acceptance"]["path"]))
    require(reviews==plan["endpoint_reviews"],"finite rebound source reviews changed")
    from neurooracle.src.kg_existing_literal_reuse import review_key
    fixed={review_key(e["claim_id"],ch["side"]) for e in plan["events"] for ch in e["changes"]}
    require(len(fixed)==plan["changed_endpoints"]==305 and fixed==set(reviews),"finite endpoint scope differs")
    require(plan["changed_endpoints"]+plan["held_endpoints"]==plan["expanded_review_endpoints"]==2511
        and plan["changed_claims"]==304 and plan["reused_existing_nodes"]==220,"queue/reuse scope differs")
    require(not plan["new_literals"] and plan["added_literal_nodes"]==0,"no new concept is authorized")
    require(plan["original_watchlist_449_repaired"]==0 and plan["original_watchlist_449_remaining_after"]==3
        and plan["source_full_sha_verified"] and plan["source_census_full_sha_verified"],"historic/current proof differs")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R55唯一完整名称已有节点复用：全连接范围与独立全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="existing_literal_reuse",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
...
def validate():
    ...
    reused_targets={r["target_id"] for r in plan["endpoint_reviews"].values()}; source_incidences=defaultdict(list)
    ...
    for cid,row in original_nodes.items():
        event,out=reviewed_claim(row,events[cid]["changes"],plan["gene_witnesses"],plan["endpoint_reviews"])
        require(all(events[cid][k]==v for k,v in event.items()),"independent gene-boundary gate differs")
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,publication_status_witnesses_validated=True,
        finite_existing_whole_name_reuse_and_current_references_reproduced=True,complete_existing_target_incidence_scope_reproduced=True,
        no_new_concept_nodes=True,no_existing_concept_metadata_change=True,no_inferred_literal_semantic_type=True,
        exact_approved_literal_endpoint_transform=True,inverse_reproduces_all_source_node_and_edge_record_digests=True,
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="existing_literal_reuse" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    receipt=dict(status="CURRENT_EXISTING_LITERAL_REUSE_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R55已有完整名称节点复用及共享索引已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R55_ACCEPTED_CONTINUE_EXISTING_IDENTITY_AND_SCIENTIFIC_SCOPE_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["305处端点复用220个已有完整名称节点，无新增节点；历史449仍剩3处海马定义。",
            "保留分子、药物、未声明类型、同名/别名/大小写及混合引用待核；不做模糊合并。",
            "17条旧语义及产前、BACE1、ACC科学范围继续保留；无模型训练或正式full_v2同步。"])
    summary=dict(graph=graph,original_watchlist=449,original_watchlist_remaining_before=3,
        ...
        expanded_repaired_this_batch=plan["changed_endpoints"],
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R55",
        latest_completed_runtime_batch="R55",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="existing_literal_reuse","unexpected active batch")
