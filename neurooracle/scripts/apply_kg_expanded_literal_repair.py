"""R53 finite non-molecular complete identities with independent full source inversion."""
import argparse
...
from apply_kg_reviewed_literal_reuse import FILES as PREVIOUS_FILES
from plan_kg_expanded_literal_repair import inspection_bridge, classification_inputs
...
from neurooracle.src.kg_expanded_literal_repair import (literal_node, reviewed_claim,
...
OUTPUT=journal.OUTPUT/"round53_expanded_literal_repair"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".expanded-literal.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES,Path(__file__),*[journal.REPO/p for p in (
    "neurooracle/src/kg_expanded_literal_repair.py","neurooracle/scripts/plan_kg_expanded_literal_repair.py",
    "neurooracle/scripts/classify_kg_expanded_literal_candidates.py","neurooracle/scripts/inspect_kg_expanded_literal_scope.py",
    "neurooracle/tests/test_kg_expanded_literal_repair.py","neurooracle/tests/test_kg_expanded_literal_plan.py",
    "neurooracle/tests/test_kg_expanded_literal_pipeline.py","neurooracle/tests/test_kg_expanded_literal_classification.py",
    "neurooracle/tests/test_kg_expanded_literal_scope_inspection.py",
    "neurooracle/scripts/report_kg_reviewed_literal_reuse.py","neurooracle/tests/test_kg_reviewed_literal_reuse_report.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=1060 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
...
def load_plan(baseline):
    ...
    require(plan["status"]=="REVIEWED_NOT_APPLIED" and plan["version"]=="kg.expanded_nonmolecular_literals.v1"
    ...
    for item in [*plan["code"],*[plan[k] for k in ("remaining_queue","source_review","classification","prior_identity_plan",
        "existing_name_collision_evidence","decline_evidence")]]:small_check(item)
    class_fp,classification,inspection,reviews=classification_inputs()
    require(class_fp==plan["classification"] and classification["source_inspection"]==plan["source_review"],"classification binding differs")
    inspection_bridge(inspection,baseline,journal.read_json(baseline["current_acceptance"]["path"]))
    require(reviews==plan["endpoint_reviews"],"finite source reviews changed")
    from neurooracle.src.kg_expanded_literal_repair import review_key
    fixed={review_key(e["claim_id"],ch["side"]) for e in plan["events"] for ch in e["changes"]}
    require(len(fixed)==plan["changed_endpoints"] and fixed<=set(reviews)
        and all(reviews[k]["literal_candidate_gate"] is None for k in fixed),"finite changed scope differs")
    require(plan["changed_endpoints"]+plan["held_endpoints"]==plan["expanded_review_endpoints"]==5490
        and 0<plan["changed_endpoints"]<=classification["candidate_endpoints"]==2979,"queue scope differs")
    require(plan["original_watchlist_449_repaired"]==0 and plan["original_watchlist_449_remaining_after"]==3
        and plan["source_full_sha_verified"] and plan["source_census_full_sha_verified"],"historic scope/current proof differs")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R53扩大队列临床与测量完整名称身份纠正：全引用与独立全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="expanded_literal_repair",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
...
def validate():
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,publication_status_witnesses_validated=True,
        finite_nonmolecular_allowlist_and_current_references_reproduced=True,no_inferred_literal_semantic_type=True,
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="expanded_literal_repair" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    receipt=dict(status="CURRENT_EXPANDED_LITERAL_REPAIR_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R53扩大队列非分子完整描述身份及共享索引已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R53_ACCEPTED_CONTINUE_EXISTING_IDENTITY_AND_SCIENTIFIC_SCOPE_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["扩大队列临床/测量完整名称已有限批量纠正；历史449仍剩3处海马定义，本批不混记历史修复数。",
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R53",
        latest_completed_runtime_batch="R53",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="expanded_literal_repair","unexpected active batch")
