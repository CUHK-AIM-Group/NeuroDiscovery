"""R50 finite existing-name reuse, mouse protein identity and three neutral old abouts."""
import argparse
...
from apply_kg_historic_literal_repair import FILES as PREVIOUS_FILES
from plan_kg_reviewed_literal_reuse import inspection_bridge, adjusted_ordinal, original_ordinal
...
from neurooracle.src.kg_reviewed_literal_reuse import (literal_node, reviewed_claim,
    reviewed_edges, edge_owner, apply_edge, reverse_claim, split_owned, candidate_retirement_proof, public_scope_proof)
...
OUTPUT=journal.OUTPUT/"round50_literal_scope_reuse"
SOURCE=journal.OUTPUT/"round23_source_deletion_candidate/knowledge_graph.candidate.json"
TEMP=SOURCE.with_name(SOURCE.name+".literal-scope-reuse.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES, Path(__file__), *[journal.REPO/p for p in (
    "neurooracle/scripts/plan_kg_reviewed_literal_reuse.py", "neurooracle/src/kg_reviewed_literal_reuse.py",
    "neurooracle/tests/test_kg_reviewed_literal_reuse.py", "neurooracle/tests/test_kg_reviewed_literal_reuse_pipeline.py",
    "neurooracle/tests/test_kg_reviewed_literal_reuse_plan.py",
    "neurooracle/scripts/inspect_kg_remaining_scoped_literals.py", "neurooracle/tests/test_kg_remaining_scoped_literal_inspection.py",
    "neurooracle/scripts/fetch_kg_remaining_scope_sources.py", "neurooracle/tests/test_kg_remaining_scope_public_sources.py",
    "neurooracle/scripts/report_kg_historic_literal_repair.py", "neurooracle/tests/test_kg_historic_literal_report.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=980 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)


class LiteralTransform:
    """Keep every original record digest except the three approved old abouts."""
    def __init__(self,plan):
        self.plan=plan
        self.events={e["claim_id"]:e for e in plan["events"]}
        self.edges={e["ordinal"]:e for e in plan["edge_events"]}
        self.removed={e["ordinal"]:e for e in plan["retired_branches"]}
        ...
        require(set(self.removed)==set(plan["removed_edge_ordinals"]) and not set(self.edges)&set(self.removed),"changed/deleted edge overlap")
        self.digests={k:hashlib.sha256() for k in ("nodes","edges")}
        self.originals={};self.refs=defaultdict(list);self.seen_edges=set();self.seen_targets=set();self.seen_removed=set()
        self.appended=False
    ...
    def records(self,records):
        for kind,key,row in records:
            if kind=="edge" and not self.appended:
                ...
                self.appended=True
            is_removed=kind=="edge" and int(key) in self.removed
            if kind!="metadata" and not is_removed:self.digests[kind+"s"].update(compact(row).encode()+b"\n")
            out=row
            if kind=="node":
                ...
                if key in self.events:
                    ...
                    reproduced,out=reviewed_claim(row,event["changes"],self.plan["gene_witnesses"],self.plan["reviewed_claims"],self.plan["public_scope_proof"])
                ...
                if is_removed:
                    require(digest(row)==self.removed[int(key)]["edge_sha256"] and owner==self.removed[int(key)]["claim_id"],"unapproved removed branch")
                    self.seen_removed.add(int(key));continue
                if int(key) in self.edges:
                    out=apply_edge(row,self.edges[int(key)]);self.seen_edges.add(int(key))
                key=adjusted_ordinal(int(key),self.plan["removed_edge_ordinals"])
            yield kind,key,out
        require(self.appended and set(self.originals)==set(self.events) and self.seen_edges==set(self.edges),"changed record scope incomplete")
        require(self.seen_targets==set(self.plan["existing_targets"]) and self.seen_removed==set(self.removed),"target/removal witnesses incomplete")
        reproduced=[];retired=[]
        for cid,row in self.originals.items():
            ...
            _,extra=split_owned(cid,row,self.refs[cid]);retired.extend(extra)
        for e in reproduced:e["current_ordinal"]=adjusted_ordinal(e["ordinal"],self.plan["removed_edge_ordinals"])
        for e in retired:e["current_kept_ordinal"]=adjusted_ordinal(e["kept_ordinal"],self.plan["removed_edge_ordinals"])
        require(sorted(reproduced,key=lambda e:e["ordinal"])==self.plan["edge_events"],"complete owned edge review differs")
        require(sorted(retired,key=lambda e:e["ordinal"])==self.plan["retired_branches"],"complete retired-branch review differs")


class IndependentLiteralProof:
    """Validate current owned closures and reconstruct only neutral old abouts."""
    def __init__(self,plan,expected_digests):
        self.plan=plan;self.expected=expected_digests
        self.events={e["claim_id"]:e for e in plan["events"]}
        self.edges={e["current_ordinal"]:e for e in plan["edge_events"]}
        self.new={r["id"]:r for r in plan["new_literals"]}
        self.inverse={k:hashlib.sha256() for k in ("nodes","edges")}
        self.originals={};self.current={};self.refs=defaultdict(list);self.current_refs=defaultdict(list)
        self.seen_edges=set();self.seen_new=set();self.seen_targets=set()
    ...
    def records(self,records):
        for kind,key,row in records:
            ...
            if kind=="node":
                if key in self.new:
                    require(row==literal_node(self.new[key]["name"]) and digest(row)==self.new[key]["node_sha256"],"new node definition differs")
                    self.seen_new.add(key);original=None
                elif key in self.events:
                    original=reverse_claim(row,self.events[key]);self.originals[key]=original;self.current[key]=row
                if key in self.plan["existing_targets"]:
                    require(digest(row)==self.plan["existing_targets"][key],"existing node changed");self.seen_targets.add(key)
            elif kind=="edge":
                ordinal=int(key);old_ordinal=original_ordinal(ordinal,self.plan["removed_edge_ordinals"])
                if ordinal in self.edges:
                    event=self.edges[ordinal];require(event["ordinal"]==old_ordinal,"current/old edge ordinal differs")
                    original=apply_edge(row,event,reverse=True);self.seen_edges.add(ordinal)
                owner=edge_owner(original)
                if owner in self.events:
                    self.refs[owner].append((old_ordinal,original));self.current_refs[owner].append((ordinal,row))
            if kind!="metadata" and original is not None:self.inverse[kind+"s"].update(compact(original).encode()+b"\n")
            yield kind,key,row

    def verify(self):
        require(set(self.originals)==set(self.events) and self.seen_edges==set(self.edges) and self.seen_new==set(self.new),"independent repair coverage differs")
        require(self.seen_targets==set(self.plan["existing_targets"]),"independent existing-node scope differs")
        require({k:h.hexdigest() for k,h in self.inverse.items()}==self.expected,"inverse kept-record digest differs")
        for item in self.plan["retired_branches"]:
            cid=item["claim_id"]
            require(candidate_retirement_proof(cid,self.current[cid],self.current_refs[cid],item),"candidate old branch proof differs")
            canonical=[row for ordinal,row in self.refs[cid] if ordinal==item["kept_ordinal"]]
            require(len(canonical)==1 and digest(canonical[0])==item["kept_edge_sha256"],"canonical source branch differs")
            old=deepcopy(canonical[0]);old["target_id"]=item["obsolete_target"]
            require(digest(old)==item["edge_sha256"],"neutral removed branch reconstruction differs")
            self.refs[cid].append((item["ordinal"],old));self.refs[cid].sort(key=lambda r:r[0])
        reproduced=[];retired=[]
        for cid,row in self.originals.items():
            event,out=reviewed_claim(row,self.events[cid]["changes"],self.plan["gene_witnesses"],self.plan["reviewed_claims"],self.plan["public_scope_proof"])
            require(all(self.events[cid][k]==v for k,v in event.items()),"independent finite review differs")
            reproduced.extend(reviewed_edges(cid,row,out,self.refs[cid]));_,extra=split_owned(cid,row,self.refs[cid]);retired.extend(extra)
        for e in reproduced:e["current_ordinal"]=adjusted_ordinal(e["ordinal"],self.plan["removed_edge_ordinals"])
        for e in retired:e["current_kept_ordinal"]=adjusted_ordinal(e["kept_ordinal"],self.plan["removed_edge_ordinals"])
        require(sorted(reproduced,key=lambda e:e["ordinal"])==self.plan["edge_events"],"independent current owned references differ")
        require(sorted(retired,key=lambda e:e["ordinal"])==self.plan["retired_branches"],"independent three old branches differ")
...
def load_plan(baseline):
    ...
    require(plan["status"]=="REVIEWED_NOT_APPLIED" and plan["version"]=="kg.reviewed_literal_reuse.v1"
        and plan["relation_grouping"]==POLICY,"wrong repair policy")
    ...
    for item in [*plan["code"],*[plan[k] for k in ("remaining_queue","source_review","public_manifest","public_source",
        "protein_source","prior_disjoint_identity_plan","protein_collision_evidence")]]:small_check(item)
    inspection=journal.read_json(plan["source_review"]["path"])
    inspection_bridge(inspection,baseline,journal.read_json(baseline["current_acceptance"]["path"]),
        journal.read_json(plan["prior_disjoint_identity_plan"]["path"]))
    for item in [inspection["code"],*inspection["artifacts"].values()]:small_check(item)
    require(public_scope_proof(Path(plan["public_source"]["path"]).read_text(encoding="utf8"),
        journal.read_json(plan["protein_source"]["path"]))==plan["public_scope_proof"],"public species proof differs")
    from neurooracle.src.kg_reviewed_literal_reuse import SPECS,PRENATAL
    reviewed={r["claim_id"]:r for r in rows(inspection["artifacts"]["CURRENT_CLAIM_PROJECTIONS.jsonl"]["path"]) if r["claim_id"] in SPECS}
    require(reviewed==plan["reviewed_claims"] and set(reviewed)==set(SPECS)=={e["claim_id"] for e in plan["events"]},"finite reviews changed")
    require(plan["changed_claims"]==plan["changed_endpoints"]==21 and plan["original_watchlist_repaired"]==19
        and plan["nonhistoric_complete_name_reassignments"]==2,"identity scope differs")
    require(len(plan["removed_edge_ordinals"])==3 and plan["removed_edge_ordinals"]==sorted(set(plan["removed_edge_ordinals"])),"neutral branch scope differs")
    require(plan["held_endpoints"]+19==plan["expanded_review_endpoints"] and plan["partial_scientific_scope_identity_only"]==[PRENATAL]
        and plan["all_scientific_scope_holds_remain_open"] and plan["source_full_sha_verified"]
        and plan["source_census_full_sha_verified"],"source or scientific-scope gate differs")
    return fp,plan
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R50完整名称复用及物种身份纠正：精确三条旧分支退役与全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="reviewed_literal_reuse",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
    ...
    progress("BUILD_LITERAL_IDENTITY_AND_EXACT_REFERENCES",claims=plan["changed_claims"],new_literals=plan["added_literal_nodes"])
    try:
...
def validate():
    ...
    inverse=IndependentLiteralProof(plan,state["source_record_digests"])
    paper_sigs,shared_ids=set(),set()
    ...
    def independent(records):
        nonlocal matched
        for kind,key,row in inverse.records(records):
            if kind!="metadata":coverage.add("node/"+node_class(key,row) if kind=="node" else "edge/all",row)
            if kind=="node" and key.startswith("CLM:"):
    ...
    checks.update(verified_identity_proofs_complete=True,paper_identity_witnesses_validated=True,publication_status_witnesses_validated=True,
        finite_same_name_and_species_identity_scope_reproduced=True,three_neutral_obsolete_about_branches_reproduced=True,
        exact_approved_literal_endpoint_transform=True,inverse_reproduces_all_kept_source_node_and_edge_record_digests=True,
        ...
        no_existing_concept_or_claim_deletion=True,no_existing_concept_metadata_change=True,no_new_scientific_assertion_edges=True,authority_verified_claims=proof.claim_status["verified"])
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="reviewed_literal_reuse" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    changed_by_id={ch["claim_id"]:ch for ch in plan["events"]}
    overlap=set(changed_by_id)&set(scope["current_claim_hashes"])
    require(overlap==set(plan["partial_scientific_scope_identity_only"]),"scientific scope overlap differs")
    for cid in overlap:
        require(scope["current_claim_hashes"][cid]==changed_by_id[cid]["claim_sha256"],"held scientific claim changed")
        scope["current_claim_hashes"][cid]=changed_by_id[cid]["current_node_sha256"]
    scope["identity_repairs_without_scientific_scope_closure"]=plan["partial_scientific_scope_identity_only"]
    scope.setdefault("resolved",{})["mouse_VCP_protein_species_identity"]="CLM:efe7514ed7b72a53"
    scope["source_resolution_limits"]="Prenatal subject identity and obsolete about resolved only; compound object, BACE1 and ACC scientific scopes remain open. Existing anchors/detail records unchanged."
    ...
    changed_counts=dict(claim_nodes=plan["changed_claims"],endpoint_ids=plan["changed_endpoints"],edge_records=plan["changed_edges"],
        ...
        retired_obsolete_about_edges=3,historic_gene_endpoints=19,other_complete_name_reassignments=2)
    receipt=dict(status="CURRENT_REVIEWED_LITERAL_REUSE_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R50现有完整名称复用、小鼠蛋白物种身份及三条旧分支已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R50_ACCEPTED_CONTINUE_SCIENTIFIC_SCOPE_AND_EXPANDED_ENDPOINT_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["历史449队列剩3处海马测量范围；扩大队列独立记账，不把待核当确认错误。",
            "现有源锚点及详情未删除；端点身份修复不代表科学结论已验真。",
            "17条旧语义及产前复合客体、BACE1、ACC继续保留；无模型训练或正式full_v2同步。"])
    summary=dict(graph=graph,original_watchlist=449,original_watchlist_remaining_before=22,
        original_watchlist_repaired_this_batch=plan["original_watchlist_repaired"],
        original_watchlist_remaining=plan["original_watchlist_remaining_after"],
    ...
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R50",
        latest_completed_runtime_batch="R50",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="reviewed_literal_reuse","unexpected active batch")
