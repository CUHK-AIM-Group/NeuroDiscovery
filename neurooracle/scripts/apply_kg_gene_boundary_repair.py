"""R43 full-imaging identity repair for canonical gene word-interior alias hits."""
import argparse
...
from apply_kg_bulk_cleanup import FILES as PREVIOUS_FILES
...
from neurooracle.src.kg_gene_boundary_repair import (literal_node, reviewed_claim,
...
TEMP=SOURCE.with_name(SOURCE.name+".gene-boundary.tmp")
...
FILES=list(dict.fromkeys([*PREVIOUS_FILES, Path(__file__), *[journal.REPO/p for p in (
    "neurooracle/scripts/plan_kg_gene_boundary_repair.py", "neurooracle/src/kg_gene_boundary_repair.py",
    "neurooracle/tests/test_kg_gene_boundary_repair.py", "neurooracle/tests/test_kg_gene_boundary_pipeline.py",
    "neurooracle/scripts/audit_kg_all_gene_endpoints.py", "neurooracle/tests/test_kg_all_gene_endpoint_census.py")]]))
...
def test_evidence():
    ...
    require(counts["tests"]==len(seen)>=750 and not any(counts[k] for k in ("failures","errors","skipped")),"complete distinct passing regression required")
    return dict(counts)
        ...
        for kind,key,row in records:
            ...
            if kind=="node":
                ...
                if key in self.events:
                    ...
                    reproduced,_=reviewed_claim(row,event["changes"],self.plan["gene_witnesses"])
...
def load_plan(baseline):
    ...
    require(plan["version"]=="kg.gene_boundary_repair.v1" and plan["relation_grouping"]==POLICY,"wrong repair policy")
    small_check(plan["source_review"])
    review=journal.read_json(plan["source_review"]["path"])
    require(review["graph"]==baseline["current_graph"],"gene census is not current")
...
def build():
    ...
    code=bindings()
    campaign=dict(baseline,status="MANUAL_ACTIVE",phase="R43整批修复单词内部基因别名误命中：全引用及全图验收",updated_at=journal.utc_now(),
        active_process=dict(kind="gene_boundary_repair",pid=os.getpid(),state=str(OUTPUT/"RUN_STATE.json")))
...
def validate():
    ...
    for cid,row in original_nodes.items():
        event,out=reviewed_claim(row,events[cid]["changes"],plan["gene_witnesses"])
...
def apply():
    ...
    require(campaign["current_graph"]==baseline["current_graph"] and campaign["active_process"]["kind"]=="gene_boundary_repair" and campaign["active_process"]["pid"]==os.getpid(),"writer/source advanced")
    ...
    receipt=dict(status="CURRENT_GENE_BOUNDARY_REPAIR_APPLIED",relation_grouping=POLICY,at=journal.utc_now(),graph=graph,counts=result["counts"],changed=changed_counts,
    ...
    campaign.update(status="COMPLETED",phase="R43完整影像指标、单词内部基因误命中及共享索引已验收",active_process=None,updated_at=receipt["at"],current_graph=graph,
        ...
        continuation_status="R43_ACCEPTED_CONTINUE_SCIENTIFIC_AND_OTHER_ENDPOINT_REVIEW",
        gene_review_queue_interpretation="扩大后的待核队列，包含R42词面筛查候选，不是已确认错误数",
        next_steps=["已整批修复明确影像指标的单词内部别名误命中；剩余数含新发现待核候选，不是全为错误。",
            "保留类型/分子机制/别名身份/混合引用待核；不进行名称截短或模糊合并。",
            "21语义、1结构待核及书目冲突继续保留；不涉及模型训练或正式full_v2同步。"])
    journal.atomic_json(journal.OUTPUT/"CAMPAIGN.json",campaign)
    night=journal.read_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json")
    night.update(active_process=None,latest_acceptance=campaign["current_acceptance"],latest_census=census_fp,latest_completed_batch="R43",
        latest_completed_runtime_batch="R43",latest_runtime_acceptance=campaign["current_runtime_acceptance"])
    journal.atomic_json(journal.OUTPUT/"NIGHT_WINDOW_20260910.json",night)
...
def resume():
    ...
    require(c["current_graph"]==state["baseline"]["current_graph"] and c["active_process"]["kind"]=="gene_boundary_repair","unexpected active batch")
