...
VERSION = 'kg.multipaper_claims.r87.v1'
APPROVED_RULESET_DIGEST = '28a55a28fb453f3ae4fe20d0d0b5a5fa61c72aa9fdcba863e8d398e9f2768bac'
...
class MultipaperTransform(RootTransform):
    def __init__(self, claims, edges, deleted_claims, deleted_edges, new_nodes, entity_events):
        if deleted_claims or deleted_edges or entity_events:
            raise ValueError('R87 preserves all existing entities and observations')
...
def apply_multipaper_reimport(kg,claim):
    policy=kg.serialization_metadata.get('multipaper_claim_aggregation')
