...
VERSION = 'kg.multipaper_claims.r84.v1'
APPROVED_RULESET_DIGEST = 'c7b57b863e9d1e2bbda78e3ef5beab8dc27e16768a8b49d9827c0dcc565af6f8'
...
class MultipaperTransform(RootTransform):
    def __init__(self, claims, edges, deleted_claims, deleted_edges, new_nodes, entity_events):
        if deleted_claims or deleted_edges or entity_events:
            raise ValueError('R84 preserves all existing entities and observations')
...
def apply_multipaper_reimport(kg,claim):
    policy=kg.serialization_metadata.get('multipaper_claim_reconciliation')
