...
VERSION = 'kg.accepted_claim_layer.v5'
...
def project(base_relations, base_dossiers, layer):
    ...
    supplemental = load_support_records(layer)
    used_supplemental = set()
    applied_reviews = set()
    for group in layer['groups']:
        ...
        for obs in observations:
            ...
            require(obs['source_identity']['paper_key'] == 'pmid:' + review['pmid'], 'Review belongs to another verified paper')
            original_pmid = obs['claim']['source_paper'].get('pmid')
            if original_pmid not in (None, ''):
                require(str(original_pmid) == review['pmid'], 'Review disagrees with original PMID')
            # An absent original PMID stays absent. Its verified identity was
            # established by the sealed original DOI/PMCID/title authority route.
        ...
        for entry in supplemental.get(target_id, []):
            obs = entry['observation']; cid = obs['claim_id']
            require(set(entry['original_claim_ids']) <= ids, 'Supplemental evidence is bound outside the complete original claim scope')
            require(cid not in ids and cid not in used_supplemental, 'Supplemental evidence collides with original or repeated source evidence')
            require({k: obs['claim'][k] for k in FIELDS} == target, 'Supplemental evidence targets a different proposition')
            supplemental_ids.add(cid); used_supplemental.add(cid); observations.append(deepcopy(obs))
        ...
        details[target_id] = dict(shared_claim_id=target_id, claim=deepcopy(target), original_claim_ids=sorted(ids),
                                 supplemental_observation_ids=sorted(supplemental_ids),original_observation_count=len(ids),
    ...
    require(used_supplemental == set(layer.get('supplemental_source_records', {})), 'Unused or unassigned supplemental source record')
...
class AcceptedClaimLayer(AcceptedClaimEvidence):
    ...
    def _ensure_current(self):
        ...
        if fp:
            ...
            require(layer.get('schema') in {VERSION, 'kg.accepted_claim_layer.v4', 'kg.accepted_claim_layer.v3', 'kg.accepted_claim_layer.v2', 'kg.accepted_claim_layer.v1'} and layer.get('status') == 'ACCEPTED', 'Claim layer is not accepted')
            ...
            from core.web.claim_layer_extension_v5 import extend_base
            ...
            for rid, relation in relations.items():
                ...
                ids = sorted(o['claim_id'] for o in dossier['observations'] if o.get('observation_origin') != ORIGIN)
                supplemental_ids = sorted(o['claim_id'] for o in dossier['observations'] if o.get('observation_origin') == ORIGIN)
                row = dict(shared_claim_id=rid, claim={k: relation[k] for k in FIELDS},
                           original_claim_ids=ids, observation_count=len(dossier['observations']), original_observation_count=len(ids),
                           supplemental_observation_ids=supplemental_ids,
