...
## User Study 工作区统一（2026-09-15）

新完整成果评审沿用客户端 `workspace-tokens.css`，并通过独立的
`study-workspace.css` / `study-workspace.js` 表现层统一两代研究页面：

- 嵌入模式隐藏独立 Discovery 品牌和重复版本栏；答题只保留一条工具栏，
  包含返回、可展开的阶段说明、继续码、导出、暂停及语言切换。
  阶段说明支持 Escape / 外部点击关闭。独立浏览器入口保留自身身份。
- 入口采用平面双栏，不叠加悬浮网页卡片。答题区为材料目录、证据、评分三栏；
  证据与评分独立滚动，底部保存与翻页始终可达。窄于 820px 时改为单列，
  目录横向滚动；只在数据成功保存后的翻页重置阅读位置。
- 深浅主题、表单、按钮、对话框、焦点提示与客户端共享。字号使用文本缩放，
  不再缩放整个 iframe 导致布局变窄；独立滚动仍计入原有活动时长条件。
- 保留更新后的所有问卷材料、选项、匿名与同意说明、计分、历史版本、结果显示
  边界、随机顺序、保存/恢复/导出流程。外观消息仍仅信任同源父页面，切换主题
  与语言不重载问卷。没有修改研究材料、真实答案或启动科学实验。

验证入口 `core/web/test_study_workspace.py` 同时运行语言与交互 Node 测试，纳入
默认 pytest 和 CI。浏览器交互使用独立临时数据库、显式 `record_kind="test"` 的
合成会话；不使用真实评审者身份或答案。源码更新不等于已重新打包安装程序。

本轮相关 Python 回归 512 项通过（1 项已有 Starlette/AnyIO 弃用警告），Node
语言与交互测试 26 项通过。浏览器检查覆盖真实主框架嵌入、深浅主题、中文/英文、
390px 窄窗口与 150% 字号、合成输入自动保存、切换语言保留输入、翻页往返与返回
聊天。历史研究入口只检查外观，没有操作认证或访问真实研究结果。
